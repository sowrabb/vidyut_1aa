...
class ComprehensiveStateInfoPage extends StatelessWidget {
  const ComprehensiveStateInfoPage({super.key});

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => StateInfoStore(context.read<DemoDataService>()),
      child: const _StateInfoPageContent(),
    );
  }
}

class _StateInfoPageContent extends StatelessWidget {
  const _StateInfoPageContent();

  @override
  Widget build(BuildContext context) {
    return Consumer<StateInfoStore>(
        ...
        return Scaffold(
          appBar: ResponsiveAppBar(
            ...
            leading: store.canGoBack && ResponsiveLayout.isMobile(context)
                ...
                : null,
            automaticallyImplyLeading: store.canGoBack && !ResponsiveLayout.isMobile(context),
            actions: [
              if (store.mainPath != MainPath.selection && !ResponsiveLayout.isMobile(context))
                IconButton(
                  icon: const Icon(Icons.home),
                  onPressed: () => store.resetToSelection(),
                  tooltip: 'Start Over',
                ),
              if (ResponsiveLayout.isMobile(context) && store.mainPath != MainPath.selection)
                PopupMenuButton(
                  itemBuilder: (context) => [
                    PopupMenuItem(
          ...
          floatingActionButton: ResponsiveLayout.isMobile(context) && store.canGoNext
